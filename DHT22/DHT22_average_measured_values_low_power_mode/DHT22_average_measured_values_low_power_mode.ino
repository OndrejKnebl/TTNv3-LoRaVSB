...
* 
* Modified for DHT22 - average measured values and Low Power Mode, 3. 4. 2022
*******************************************************************************/
...
unsigned int sendDataEvery = 71;              // Send data every 71 sleep
...
void do_measure(osjob_t* j){
    ...
    while(errorDHT){                                  // There was DHT error, measureTempsAndHumi() will be called again
        measureTempsAndHumi();
    }
    
    LowPower.powerDown(SLEEP_8S, ADC_OFF, BOD_OFF);   // Sleep time = 8 seconds   

    if(countSleeps % 7 == 0){                         // Every 7 sleep (8 * 7 = 56 seconds)
        measureTempsAndHumi();
    }

    if(countSleeps % sendDataEvery == 0){             // Every 71 sleep (71 * 8 = 9.467 minutes)
        os_setCallback(&sendjob, do_send);            // Call do_send
        countSleeps = 0;                              // Zero seconds counter
    }
    countSleeps++;                                    // + 1 sleep
